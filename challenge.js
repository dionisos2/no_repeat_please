...
function getReps(ingredients) {
  ...
  for(i = 0; i < ingredients.length; i++) {
    if (currentNature === ingredients[i][1]) {
      number++;
    } else {
      currentNature = ingredients[i][1];
      if (number > 1) {
        reps.push(number);
      }
      number = 1;
...
function magicRecipeOfProtection(ingredients) {
  var reps = getReps(ingredients);
  console.log(reps);
  reps.sort(function(a,b) {
    return a < b;
  });

  result = permWithoutRepeatsM(reps, ingredients.length);
